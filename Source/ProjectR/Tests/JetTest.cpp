	...
	if (GEditor->IsPlayingSessionInEditor())
	{
		...
		if (testJet)
		{
			...
			*tickCount = *tickCount + 1;
			...
			if ( (*tickCount) > tickLimit)
			{
				test->TestTrue(TEXT("If a jet is at top speed, it should never increase it after an acceleration is added (after ticking)."), FMath::IsNearlyEqual(currentSpeed, testJet->settedTopSpeed(), 0.09f));
				testWorld->bDebugFrameStepExecution = true;
				return true;
			}
		}
