...
DEFINE_LATENT_AUTOMATION_COMMAND(FSpawnARaceRunningMakeItStartCommand);

bool FSpawnARaceRunningMakeItStartCommand::Update()
{
	if (!GEditor->IsPlayingSessionInEditor())
	{
		return false;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FARaceRunningStagStartSpawnsLapManagerTest, "ProjectR.RaceRunningStage Tests.Unit.002: start spawns a lap manager", EAutomationTestFlags::ApplicationContextMask | EAutomationTestFlags::ProductFilter)

bool FARaceRunningStagStartSpawnsLapManagerTest::RunTest(const FString& Parameters)
{
	FString testWorldName = FString("/Game/Tests/TestMaps/VoidWorld-RaceGameModeMOCK");
	...
	ADD_LATENT_AUTOMATION_COMMAND(FSpawnARaceRunningMakeItStartCommand);
